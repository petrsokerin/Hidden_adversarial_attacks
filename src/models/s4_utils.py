...
def dplr(
    ...
    assert torch.all(A.real < 1e-4) and torch.all(
        A.imag <= 0.0)  # Allow some tolerance for numerical precision on real part
...
class SSMKernel(Kernel):
    ...
    def init_dt(self):
        ...
        if self.deterministic:  # Meant for debugging
            ...
            inv_dt = torch.exp(torch.linspace(math.log(self.dt_min), math.log(self.dt_max), self.H)).unsqueeze(
                -1)  # (H 1)
        else:
...
class SSMKernelDiag(SSMKernel):
    ...
    def register_params(self, A, B, C, inv_dt, P):
        ...
        C = C.expand(torch.broadcast_shapes(C.shape, (
        1, self.H, self.N)))  # (C, H, N)  # TODO originally this was only in DPLR, check safe for Diag
...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def _setup_linear(self):
        ...
        R = (torch.eye(self.rank, dtype=A.dtype, device=A.device) + 2 * contract('r h n, h n, s h n -> h r s', Q, D,
                                                                                 P).real)  # (H R R)
    ...
    def _setup_step(self, mode='dense'):
        ...
        if mode == 'linear':
            ...
            raise NotImplementedError("DPLR Kernel step mode must be {'dense' | 'linear' | 'diagonal'}")
